...
### B. voyage-4 (Vector Generation)
- **Input Tokens:** ~200 tokens
    - Processes the generated summary from the GPT-4o-mini output.
    - Generates a 1024-dimensional embedding vector via the MongoDB AI endpoint (`https://ai.mongodb.com/v1`).
...
*Estimates based on standard pricing tiers for GPT-4o-mini and Voyage AI voyage-4.*
...
| **voyage-4 Embedding (Input)** | $0.06 | 200M × $0.06 | **$12.00** |
| **TOTAL DAILY COST** | | | **~$255.00** |

---

## 4. Key Takeaways

- **Estimated Monthly Cost:** ~$7,650
- **Efficiency:** The use of `gpt-4o-mini` for summarization and `voyage-4` for embeddings is highly cost-effective at this scale. `voyage-4` produces superior 1024-dim embeddings with stronger retrieval quality than `text-embedding-3-small` at $0.06/1M tokens.
